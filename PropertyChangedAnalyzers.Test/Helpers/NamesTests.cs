...
namespace RoslynSandbox
{
    internal class Foo
    {
            ...
            : this()
        {
            ...
            Assert.AreEqual(false, syntaxTree.GetRoot().UnderscoreFields(semanticModel, CancellationToken.None));
        }
...
namespace RoslynSandbox
{
    class Foo
    {
        ...
        public int Bar()  => _value = 1;
            ...
            var compilation = CSharpCompilation.Create("test", new[] { syntaxTree }, MetadataReferences.FromAttributes());
            var semanticModel = compilation.GetSemanticModel(syntaxTree);
            Assert.AreEqual(true, syntaxTree.GetRoot().UnderscoreFields(semanticModel, CancellationToken.None));
        }
...
namespace RoslynSandbox
{
    class Bar
    {
            ...
            foreach (var tree in compilation.SyntaxTrees)
            {
                ...
                Assert.AreEqual(true, tree.GetRoot().UnderscoreFields(semanticModel, CancellationToken.None));
            }
        }
    }
}